#!/usr/bin/env bash
# Fetch a pinned tool into tools/.deps/ when it is not already on PATH.
#
# Usage: tools/scripts/tools.sh writing-lint
...
case "${1:-}" in
  writing-lint)
    ...
    ;;
  *)
    echo "usage: tools/scripts/tools.sh writing-lint" >&2
    exit 2
    ;;
esac
